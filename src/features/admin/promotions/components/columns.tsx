...
export const columns: ColumnDef<PromotionGetManyByStore[number]>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <Badge variant={type}>{capitalizeWords(type as string)}</Badge>
            );
        },
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <div className="flex items-center gap-x-2">
                    {type === "FIXED" && (
                        <p className="line-clamp-1">{formatPrice(value)}</p>
                    )}
                    {type === "PERCENT" && (
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            if (start && end) {
                let status = { label: "Running", color: "bg-green-500" };

                if (now < start)
                    status = { label: "Upcoming", color: "bg-blue-500" };
                if (now > end)
                    status = { label: "Expired", color: "bg-gray-500" };
            ...
            return <p className="line-clamp-1 font-semibold">Infinite</p>;
        },
                        ...
                        >
                            <div className="flex flex-col gap-y-2">
                                <p className="text-xs font-semibold text-muted-foreground mb-1 uppercase tracking-wider">
                                    {capitalizeWords(
                                        snakeCaseToTitle(mode as string),
                                    )}
                                </p>
                                {items.map((name) => (
                                    <div
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <ToggleActive isChecked={isActive} id={id} storeId={storeId} />
            );
        },
    },
    {
