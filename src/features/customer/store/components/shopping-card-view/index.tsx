...
export const ShoppingCart = ({ storeId }: ShoppingCartProps) => {
    const [items, setItems] = useState<CartItem[]>(cartData.items);
    const [isRemoving, setIsRemoving] = useState<string | null>(null);

    const updateQuantity = (id: string, increment: boolean) => {
        setItems((currentItems) =>
            currentItems.map((item) => (item.id === id ? { ...item, quantity: Math.max(1, item.quantity + (increment ? 1 : -1)) } : item)),
        );
    };
                ...
                <div className="flex flex-col gap-4 w-full lg:w-96">
                    ...
                    <Card className="sticky top-4 gap-0">
                        ...
                        <CardContent className="flex flex-col gap-4">
                            ...
                            <Input className="max-h-7 mt-6" placeholder="Promo code" />

                            <Button size="lg" variant="outline" className="h-10 px-8 mt-2 w-full text-base font-medium" disabled={items.length === 0}>
                                <TagIcon className="text-center" />
                            </Button>

                            <Button size="lg" className="h-10 px-8 mt-4 w-full text-base font-medium" disabled={items.length === 0}>
                                <ShoppingBag />
                                Proceed to Checkout
                            </Button>
